...
const PriceWizard = new Scenes.WizardScene(
	...
	async (ctx) => {
		...
		const resp = await fetch(
			`https://api.coingecko.com/api/v3/simple/price?ids=${ctx.wizard.state.coinName}&vs_currencies=${ctx.wizard.state.currency}`
		);
		const data = await resp.json();

		ctx.replyWithMarkdownV2(
			`Current price of *1 ${ctx.wizard.state.coinName.toUpperCase()}* is *${
				data[ctx.wizard.state.coinName][ctx.wizard.state.currency]
			} ${ctx.wizard.state.currency.toUpperCase()}*`
		);
...
const DeployWizard = new Scenes.WizardScene(
	...
	async (ctx) => {
		if (
			...
			ctx.message.document.file_name.endsWith(".sol")
		) {
			...
			if (response.ok) {
				...
				if (ctx.message.document.file_name.includes(" ")) {
					...
					ctx.reply(
						'Enter space separated constructor arguments. If there are no constructor arguments, just send me "null"'
					);
					return ctx.wizard.next();
				}
			} else {
				ctx.reply(
					"An error occurred while fetching the file, Plesse try again."
				);
				ctx.wizard.back();
				return ctx.wizard.steps[ctx.wizard.cursor](ctx);
			}
...
Total deployment Cost: ${contract.data.gasFeePaid.replaceAll(".", "\\.")} ETH`,
								...
								[
									{
										...
										url: `https://goerli.etherscan.io/tx/${contract.data.transactionHash}`,
									},
								],
							],
						},
			...
			console.log(e);
			ctx.reply(
				"An Unknown error occurred! Don't worry, no gas fees was deducted from your account. Please try again later."
			);
			return ctx.scene.leave();
		}
	}
...
bot.start(async (ctx) => {
	ctx.replyWithSticker(
		"CAACAgIAAxkBAANFZS2HXgk5VS9PvQ7Bk2VuowABOKpsAAIFAAPANk8T-WpfmoJrTXUwBA"
	);
	ctx.reply(
		"Hi there, Welcome to Plutonium Wallet.\nUse /help command to view how to use this bot."
	);
});
...
bot.command("gas_info", async (ctx) => {
	...
	const resp = await fetch(
		`https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=${process.env.ETHERSCAN_API_TOKEN}`
	);
	const data = await resp.json();
...
bot.command("create_wallet", async (ctx) => {
	try {
		...
		if (userD.status === 200) {
			...
			ctx.replyWithMarkdownV2(
				replyMessages["CREATE_WALLET_ALREADY_FOUND"](
					userD.data.addresses[0].slice(0, 6) +
						"\\.\\.\\." +
						userD.data.addresses[0].slice(-4)
				)
			);
		} else {
			// Wallet not found
			...
			if (arr.length === 2) {
				...
				if (resp.status === 200) {
					...
					if (addResp.status === 200) {
						...
						ctx.replyWithMarkdownV2(
							replyMessages["CREATE_WALLET_SUCCESS"](resp.data.address)
						);
						ctx.deleteMessage(replyData.message_id);
					} else {
...
bot.command("view_seed_phrase", async (ctx) => {
	try {
		if (ctx.message.chat.type === "private") {
			...
			if (userD.status === 200) {
				...
				if (arr.length === 2) {
					...
					const decrypted = await decryptMnemonic(
						arr[1],
						JSON.parse(userD.data.encryptedData)
					);
					if (decrypted.status === 200) {
						// Show hidden Seed Phrase which will get deleted in 5 sec
						const msgToDelete = await ctx.replyWithMarkdownV2(
							replyMessages["VIEW_SEED_SUCCESS"](decrypted.data.mnemonic)
						);
						...
						setTimeout(() => {
							...
							ctx.replyWithMarkdownV2(
								"*The previous message was deleted for security purpose\\.*"
							);
						}, 5000);
					} else {
						// Show error message
			...
			ctx.reply(
				"This command is only available in Private Chats for security purposes."
			);
		}
	} catch (err) {
		console.log(err);
		...
		if (userD.status === 200) {
			...
			if (arr.length === 2) {
				...
				if (allTokensResp.status === 200) {
					const replyMarkupTiles = Object.keys(allTokensResp.data).map(
						(key) => [
							{
								text: `${allTokensResp.data[key].balance} ${allTokensResp.data[key].symbol} (${allTokensResp.data[key].name})`,
								url: `https://goerli.etherscan.io/token/${allTokensResp.data[key].address}`,
							},
						]
					);

					ctx.reply("All tokens owned by you are listed below:", {
						reply_markup: { inline_keyboard: replyMarkupTiles },
					});
					ctx.deleteMessage(replyData.message_id);
				} else {
...
bot.command("show_qr", async (ctx) => {
	...
	try {
		if (userD.status === 200) {
			...
			ctx.replyWithPhoto(
				{
					url: "https://quickchart.io/qr?text=0x68a146f881Ec7310b644A3Fe2B6da6fc82F22A9E&margin=2&size=300",
				},
				{
					caption:
						"Scan this QR with any dApp that supports WalletConnect, to seamlessly connect your wallet with it.",
				}
			);
			ctx.deleteMessage(replyData.message_id);
		} else {
...
bot.command("txn_history", async (ctx) => {
	...
	try {
		...
		if (userD.status === 200) {
			...
			const arr = ctx.message.text.split(" ");
			if (arr.length === 2) {
				const allTxnsResp = await getAllTxns(
					ctx.message.from.id.toString(),
					arr[1]
				);
					...
					allTxnsResp.data.results.forEach((result) => {
						...
						if (result.transfers) {
							result.transfers.forEach((item) => {
								transfers += `*${item.type.replaceAll("_", "\\_")}*\\: ${
									item.value * 10 ** -18
...
${transfers}
*From*\\: [${
							result.from.slice(0, 6) + "\\.\\.\\." + result.from.slice(-4)
						}](https://goerli.etherscan.io/address/${result.from}) 
*To*\\: [${
							result.to.slice(0, 6) + "\\.\\.\\." + result.to.slice(-4)
						}](https://goerli.etherscan.io/address/${result.to})
*At*\\: ${timestamp}`;
							...
							reply_markup: {
								inline_keyboard: [
									[
										{
											text: "View Transaction on explorer",
											url: `https://goerli.etherscan.io/tx/${result.txHash}`,
										},
									],
								],
							},
